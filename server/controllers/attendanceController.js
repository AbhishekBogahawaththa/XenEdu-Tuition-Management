...
// Mark attendance by scanning barcode
const markByBarcode = async (req, res) => {
  try {
    ...
    if (!student) {
      return res.status(404).json({ message: `No student found: ${admissionNumber}` });
    }

    // Check student is enrolled in this class
    const cls = await Class.findById(session.classId);
    if (!cls.enrolledStudents.includes(student._id)) {
  ...
  try {
    ...
    for (const record of attendance) {
      ...
      if (!classStats[className]) {
        classStats[className] = { total: 0, present: 0, absent: 0, late: 0 };
      }
