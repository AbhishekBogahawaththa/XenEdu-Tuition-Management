...
const AdminClasses = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!form.name || !form.subject || !form.grade || !form.dayOfWeek || !form.startTime || !form.endTime) {
      toast.error('Please fill all required fields including end time'); return;
    }
      ...
      <div className="space-y-6">
          ...
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
            <div className="bg-white rounded-2xl w-full max-w-2xl max-h-screen overflow-y-auto shadow-2xl">
              <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
                ...
                <button onClick={() => { setShowForm(false); resetForm(); }}><X size={22} className="text-gray-400 hover:text-gray-600"/></button>
              </div>
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 flex items-start gap-3">
                  <Calendar size={16} className="text-blue-500 mt-0.5 flex-shrink-0"/>
                  <p className="text-blue-700 text-sm">Sessions will be <strong>automatically generated</strong> for the current and next month when you create this class.</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className={labelClass}>Class Name *</label>
                    <input type="text" placeholder="e.g. Physics Grade 13 - Sinhala 2026" value={form.name}
                  ...
                  <div className="col-span-2">
                    ...
                    {form.startTime && form.endTime && (() => {
                      ...
                      return (
                        <div className="mt-2 bg-[#0d6b7a]/5 border border-[#0d6b7a]/20 rounded-lg px-4 py-2.5 flex items-center gap-3">
                          <span className="text-lg">⏱️</span>
                          <div>
                            <span className="text-[#0d6b7a] text-sm font-bold">{form.startTime} — {form.endTime}</span>
                            <span className="text-[#0d6b7a] text-sm ml-2 opacity-70">({d.label} · {d.mins} mins per session)</span>
                          </div>
                        </div>
                      );
                    })()}
                  ...
                  <div>
                    ...
                    <input type="number" value={form.maxCapacity} onChange={e => setForm({ ...form, maxCapacity: e.target.value })} className={inputClass} min="1" placeholder="40"/>
                  </div>
                  <div>
                    <label className={labelClass}>Monthly Fee (Rs.)</label>
                    <input type="number" value={form.monthlyFee} onChange={e => setForm({ ...form, monthlyFee: e.target.value })} className={inputClass} min="0" placeholder="2500"/>
                  </div>
                </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-3">
            {filtered?.map(cls => (
              <div key={cls._id} className="bg-white rounded-xl shadow-sm p-5 flex items-center justify-between hover:shadow-md transition">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1.5 flex-wrap">
                      <span className="flex items-center gap-1 text-xs text-gray-500">
                        <Clock size={11}/>{cls.schedule?.dayOfWeek} {cls.schedule?.startTime}{cls.schedule?.endTime ? ` — ${cls.schedule?.endTime}` : ''}{cls.schedule?.durationMins ? ` (${cls.schedule.durationMins} mins)` : ''}
                      </span>
                      <span className="text-gray-300">•</span>
                      <span className="flex items-center gap-1 text-xs text-gray-500"><Users size={11}/>{cls.enrolledCount}/{cls.maxCapacity} enrolled</span>
                      <span className="text-gray-300">•</span>
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${cls.availableSlots === 0 ? 'bg-red-100 text-red-600' : cls.availableSlots <= 10 ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'}`}>
                ...
                <div className="flex items-center gap-4 flex-shrink-0 ml-4">
                  ...
                  <button onClick={() => { if (window.confirm(`Delete "${cls.name}"? This cannot be undone.`)) deleteMutation.mutate(cls._id); }}
                    className="p-2 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition" title="Delete class">
                    <X size={16}/>
                  </button>
                </div>
              </div>
