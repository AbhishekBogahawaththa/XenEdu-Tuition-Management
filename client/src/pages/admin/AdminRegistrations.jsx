            ...
            <tbody>
              ...
              {filtered?.map((req) => (
                <tr key={req._id} className="border-t border-gray-100 hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4">
                    <p className="text-gray-700 font-medium">{req.parentName}</p>
                    <p className="text-xs text-gray-400">{req.parentContact}</p>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`text-xs font-bold px-3 py-1 rounded-full capitalize ${statusColors[req.status]}`}>
                      {req.status}
                    </span>
                  </td>
                    ...
                    <div className="flex gap-2">
                      ...
                      {req.status === 'pending' && (
                        <>
                          ...
                          <button
                            onClick={() => { setRejectingId(req._id); setShowRejectModal(true); }}
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6">
              ...
              {selected.status === 'pending' && (
                <div className="flex gap-3 mt-5">
                  ...
                  <button
                    onClick={() => { setRejectingId(selected._id); setShowRejectModal(true); }}
                    className="flex-1 bg-red-50 text-red-600 py-2.5 rounded-xl font-semibold hover:bg-red-100 transition"
                  >
                    Reject
                  </button>
                </div>
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
              ...
              <textarea
                ...
                rows={4}
                className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-[#1B6B5A] resize-none"
              />
