...
const PaymentRequestsTab = () => {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: (res) => {
      toast.success(`Approved! Receipt: ${res.data.receiptNumber}`);
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
          ...
          <div key={req._id} className="bg-white rounded-xl shadow-sm p-5">
            <div className="flex justify-between items-start">
              <div className="flex gap-4 items-start">
                <div className="w-10 h-10 rounded-full bg-[#1B6B5A] flex items-center justify-center text-white font-bold flex-shrink-0">
                  {req.studentId?.userId?.name?.charAt(0)}
                </div>
                <div>
                  ...
                  <p className="text-sm text-gray-500 capitalize mt-0.5">
                    Method: <span className="font-semibold">{req.method?.replace('_', ' ')}</span>
                  </p>
                  ...
                  {req.slipUrl && (
                    <a href={`http://localhost:5000${req.slipUrl}`} target="_blank" rel="noreferrer"
              ...
              <div className="text-right flex-shrink-0 ml-4">
                ...
                <span className={`text-xs font-bold px-2 py-1 rounded-full capitalize ${statusColors[req.status]}`}>
                  {req.status}
                </span>
              </div>
            </div>
            {req.status === 'pending' && (
              <div className="flex gap-3 mt-4 pt-4 border-t border-gray-100">
                <button onClick={() => approveMutation.mutate(req._id)} disabled={approveMutation.isPending}
                  className="flex-1 bg-[#1B6B5A] text-white py-2.5 rounded-lg font-semibold text-sm hover:bg-[#155a4a] transition disabled:opacity-50">
                  ✓ Approve Payment
                </button>
                <button
                  onClick={() => { const r = window.prompt('Rejection reason:'); if (r !== null) rejectMutation.mutate({ id: req._id, reason: r }); }}
                  disabled={rejectMutation.isPending}
                  className="flex-1 bg-red-50 text-red-600 py-2.5 rounded-lg font-semibold text-sm hover:bg-red-100 transition">
                  ✗ Reject
                </button>
              </div>
            )}
        ...
        {data?.requests?.length === 0 && (
          <div className="bg-white rounded-xl p-12 text-center text-gray-400">No {filter} payment requests</div>
        )}
      </div>
    </div>
  );
          ...
          {monthlyReport && (
            <div className="space-y-4">
              <div className="grid grid-cols-4 gap-4">
                {[
                  { label: 'Total Generated', value: `Rs. ${monthlyReport.summary?.totalGenerated?.toLocaleString()}`, color: 'text-gray-800' },
                  { label: 'Collected', value: `Rs. ${monthlyReport.summary?.totalCollected?.toLocaleString()}`, color: 'text-green-600' },
                  { label: 'Unpaid', value: `Rs. ${monthlyReport.summary?.totalUnpaid?.toLocaleString()}`, color: 'text-red-600' },
                  { label: 'Collection Rate', value: monthlyReport.summary?.collectionRate, color: 'text-[#1B6B5A]' },
                ].map(card => (
              ...
              <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Student</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Class</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Amount</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Due Date</th>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-3 gap-4">
                {[
                  ...
                  { label: 'Period', value: `${dateFrom} → ${dateTo}`, color: 'text-[#1B6B5A]' },
                ].map(card => (
                  <div key={card.label} className="bg-white rounded-xl p-5 shadow-sm">
                    <p className="text-gray-500 text-sm">{card.label}</p>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Payments</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Total</th>
                    </tr>
                  </thead>
                  <tbody>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Student</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Class</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Method</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Amount</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Receipt</th>
                    </tr>
                  </thead>
                  <tbody>
                    {dateReport.payments?.map((p, i) => (
                      <tr key={i} className="border-t border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="px-6 py-3 font-bold text-green-600">Rs. {p.amount?.toLocaleString()}</td>
                        <td className="px-6 py-3 text-xs text-gray-400">{p.receiptNumber}</td>
                      </tr>
                    ))}
      ...
      {reportType === 'teacher' && (
        <div className="space-y-4">
          <div className="bg-white rounded-xl shadow-sm p-5">
            <div className="grid grid-cols-3 gap-4 items-end">
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase mb-1">Select Teacher</label>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-[#1B6B5A] rounded-xl p-5 text-white flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center text-xl font-bold">
                    {teacherReport.teacher?.name?.charAt(0)}
                  </div>
                  <div>
              ...
              <div className="grid grid-cols-4 gap-4">
                {[
                  ...
                  { label: 'Generated', value: `Rs. ${teacherReport.totalGenerated?.toLocaleString()}`, color: 'text-gray-800' },
                  { label: 'Collected', value: `Rs. ${teacherReport.totalCollected?.toLocaleString()}`, color: 'text-green-600' },
                  { label: 'Collection Rate', value: `${teacherReport.collectionRate}%`, color: 'text-[#1B6B5A]' },
                ].map(card => (
                  <div key={card.label} className="bg-white rounded-xl p-5 shadow-sm">
                    <p className="text-gray-500 text-sm">{card.label}</p>
                    <p className={`text-2xl font-bold mt-1 ${card.color}`}>{card.value}</p>
              ...
              <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Class</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Students</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Fee/Month</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Generated</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Collected</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Unpaid</th>
                      <th className="text-left px-6 py-3 text-gray-500 font-semibold">Rate</th>
                    </tr>
                  </thead>
                  <tbody>
                  ...
                  <tfoot>
                    <tr className="bg-[#1B6B5A]/5 border-t-2 border-[#1B6B5A]/20">
...
const AdminFees = () => {
  ...
  const generateMutation = useMutation({
    ...
    onSuccess: (res) => {
      toast.success(`Generated ${res.data.generated} fee records for ${res.data.month}`);
      queryClient.invalidateQueries(['outstanding']);
    },
    onError: err => toast.error(err.response?.data?.message || 'Failed'),
  });
  ...
  const handleScan = async () => {
    ...
    try {
      const res = await api.get(`/scan/${scanValue.trim()}`);
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="bg-white rounded-xl p-5 shadow-sm">
                ...
                <p className="text-2xl font-bold text-gray-800 mt-1">{outstanding?.count ?? '...'}</p>
              </div>
              <div className="bg-white rounded-xl p-5 shadow-sm">
                <p className="text-gray-500 text-sm">Students Affected</p>
                <p className="text-2xl font-bold text-gray-800 mt-1">
                  {outstanding ? new Set(outstanding.fees?.map(f => f.studentId?._id)).size : '...'}
                </p>
              </div>
            </div>
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="text-left px-6 py-3 text-gray-500 font-semibold">Due Date</th>
                    <th className="text-left px-6 py-3 text-gray-500 font-semibold">Status</th>
                    <th className="text-left px-6 py-3 text-gray-500 font-semibold">Parent</th>
                  </tr>
                </thead>
                <tbody>
                  {loadingOutstanding && (
                  ...
                  {outstanding?.fees?.map(fee => (
                    <tr key={fee._id} className="border-t border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        ...
                        <p className="text-gray-400 text-xs">{fee.studentId?.parentId?.contactNumber}</p>
                      </td>
                    </tr>
                  ))}
                  {outstanding?.fees?.length === 0 && !loadingOutstanding && (
                    <tr><td colSpan="7" className="px-6 py-8 text-center text-gray-400">No outstanding fees</td></tr>
                  )}
                </tbody>
              </table>
            </div>
              ...
              <div className="flex gap-3">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1B6B5A]"
                  />
                </div>
            ...
            {scannedStudent && (
              <div className="bg-white rounded-xl shadow-sm p-6 space-y-5">
                <div className="flex items-center gap-4 pb-4 border-b border-gray-100">
                  <div className="w-12 h-12 rounded-full bg-[#1B6B5A] flex items-center justify-center text-white font-bold text-lg">
                ...
                {selectedFee && (
                  <div className="bg-[#1B6B5A]/5 border border-[#1B6B5A]/20 rounded-xl p-4 space-y-3">
                    <p className="font-semibold text-gray-800">
                      Paying for: <span className="text-[#1B6B5A]">{selectedFee.name}</span>
                    </p>
                    <div>
                    ...
                    {!selectedFee.feeRecordId && (
                      <p className="text-xs text-orange-600 flex items-center gap-1">
                        <AlertCircle size={12} /> Fee record not generated yet. Generate fees first.
                      </p>
                    )}
                  </div>
                )}
