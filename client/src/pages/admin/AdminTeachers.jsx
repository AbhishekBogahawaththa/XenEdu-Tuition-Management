      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">{data?.count ?? 0} total teachers</p>
          </div>
          <button
            onClick={() => setShowAddModal(true)}
            className="flex items-center gap-2 bg-[#1B6B5A] text-white px-4 py-2.5 rounded-xl font-semibold text-sm hover:bg-[#155a4a] transition"
          >
            <Plus size={16} />
            Add Teacher
          </button>
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="text-left px-6 py-3 text-gray-500 font-semibold">Teacher</th>
                <th className="text-left px-6 py-3 text-gray-500 font-semibold">Contact</th>
                <th className="text-left px-6 py-3 text-gray-500 font-semibold">Subjects</th>
                <th className="text-left px-6 py-3 text-gray-500 font-semibold">Classes</th>
                ...
                <tr key={t._id} className="border-t border-gray-100 hover:bg-gray-50 transition">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-gray-800">{t.userId?.name}</p>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      {t.subjectExpertise?.slice(0, 2).map(s => (
                        <span key={s} className="bg-blue-100 text-blue-700 text-xs px-2 py-0.5 rounded-full font-medium">
                          {s}
                        </span>
                      ))}
                  ...
                  <td className="px-6 py-4">
                    <span className="bg-purple-100 text-purple-700 text-xs font-bold px-2 py-1 rounded-full">
                      {t.assignedClasses?.length ?? 0} classes
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`text-xs font-bold px-3 py-1 rounded-full ${
                      t.isAvailable ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'
                    }`}>
            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 max-h-screen overflow-y-auto">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1B6B5A]"
                      placeholder="Leave blank for XenEdu@1234"
                    />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1B6B5A]"
                    placeholder="BSc Physics, University of Colombo"
                  />
                </div>
                    ...
                    {SUBJECTS.map(s => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                          form.subjectExpertise.includes(s)
              ...
              <div className="flex gap-3 mt-5">
                <button
                  ...
                  className="flex-1 bg-[#1B6B5A] text-white py-2.5 rounded-xl font-semibold hover:bg-[#155a4a] transition"
                >
                  {createMutation.isPending ? 'Creating...' : 'Create Teacher'}
                </button>
                <button
                  onClick={() => setShowAddModal(false)}
                  className="flex-1 bg-gray-100 text-gray-600 py-2.5 rounded-xl font-semibold hover:bg-gray-200 transition"
                >
                  Cancel
                </button>
              </div>
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
              ...
              <div className="flex items-center gap-4 mb-5">
                <div className="w-14 h-14 rounded-full bg-[#1B6B5A] flex items-center justify-center text-white text-xl font-bold">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selected.subjectExpertise?.map(s => (
                      <span key={s} className="bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded-full font-medium">{s}</span>
                    ))}
                  </div>
                </div>
              ...
              <button
                ...
                className="w-full mt-4 bg-gray-100 text-gray-600 py-2.5 rounded-xl font-semibold hover:bg-gray-200 transition"
              >
                Close
              </button>
            </div>
