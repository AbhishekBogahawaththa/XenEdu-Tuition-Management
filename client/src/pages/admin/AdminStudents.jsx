        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-gray-50">
              <tr>
                <th className="text-left px-6 py-3 text-gray-500 font-semibold">Admission No</th>
                <th className="text-left px-6 py-3 text-gray-500 font-semibold">Student</th>
                <th className="text-left px-6 py-3 text-gray-500 font-semibold">School</th>
                ...
                <th className="text-left px-6 py-3 text-gray-500 font-semibold">Medium</th>
            ...
            <tbody>
              ...
              {filtered?.map((s) => (
                <tr key={s._id} className="border-t border-gray-100 hover:bg-gray-50 transition">
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-xs font-bold px-3 py-1 rounded-full capitalize ${statusColors[s.status]}`}>
                      {s.status}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex gap-2">
                      ...
                      {s.status === 'active' && (
                        <button
                          onClick={() => statusMutation.mutate({ id: s._id, status: 'suspended' })}
                      ...
                      {s.status === 'suspended' && (
                        <button
                          onClick={() => statusMutation.mutate({ id: s._id, status: 'active' })}
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  <p className="text-xs font-bold text-gray-400 uppercase mb-3">
                    Enrolled Classes ({selected.enrolledClasses?.length ?? 0})
                  </p>
                  {selected.enrolledClasses?.length === 0 && (
              ...
              <div className="flex gap-3 mt-5">
                {selected.status === 'active' && (
                  <button
                    ...
                    className="flex-1 bg-yellow-50 text-yellow-700 py-2.5 rounded-xl font-semibold hover:bg-yellow-100 transition border border-yellow-200"
                  >
                    Suspend Student
                  </button>
                )}
                {selected.status === 'suspended' && (
                  <button
                    ...
                    className="flex-1 bg-green-50 text-green-700 py-2.5 rounded-xl font-semibold hover:bg-green-100 transition border border-green-200"
                  >
                    Activate Student
                  </button>
                )}
