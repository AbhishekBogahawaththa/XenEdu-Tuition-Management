...
import { CreditCard, BookOpen, AlertTriangle, CheckCircle, Clock } from 'lucide-react';
        ...
        {data?.children?.map((child, i) => (
          <div key={i} className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#1B6B5A] flex items-center justify-center text-white font-bold">
                    ...
                    <div key={j} className="px-5 py-4">
                      <div className="flex justify-between items-center mb-2">
                        <div>
                          ...
                          <p className="text-xs text-gray-400">
                            {att.hall} • {att.schedule?.dayOfWeek}
                          </p>
                        </div>
                        <div className="text-right">
                          <p className={`text-lg font-bold ${
                            att.atRisk ? 'text-red-500' : 'text-green-600'
                          }`}>
                            {att.percentage}
                          </p>
                          {att.atRisk && (
                      ...
                      {att.percentage !== 'No sessions yet' && (
                        <div className="w-full h-2 bg-gray-100 rounded-full">
                          <div
                            className={`h-2 rounded-full ${att.atRisk ? 'bg-red-500' : 'bg-green-500'}`}
                            style={{ width: att.percentage }}
                          />
                        </div>
                      )}
                      <p className="text-xs text-gray-400 mt-1">
                        {att.presentCount} / {att.totalSessions} sessions attended
                      </p>
                    </div>
                  ))}
                </div>
